...
var KEY = "treino_videos_ios_v6_1_premium";
...
function saveProfile(){
  ...
  var g = document.getElementById("profileGoal");
  if(n && g){
...
function doneCountToday(){
  var ex = todayExercises(), checks = getTodayChecks(), count = 0, i;
  for(i=0;i<ex.length;i++){ if(checks[i]) count++; }
  return count;
}
...
function completedWorkoutDaysCount(){
  var vals = Object.values(state.workoutDays), count = 0, i;
  for(i=0;i<vals.length;i++){ if(vals[i]) count++; }
  return count;
}
function currentWeekCount(){
  ...
  var entries = Object.entries(state.workoutDays), count = 0, i;
  for(i=0;i<entries.length;i++){ if(entries[i][1] && entries[i][0] >= start) count++; }
  return count;
}
...
function achievements(){
  ...
  var habits = Object.values(state.habits), perfect = 0, i;
  for(i=0;i<habits.length;i++){ if(habits[i].dieta && habits[i].agua && habits[i].sono) perfect++; }
  return [
...
function toggleExercise(i){
  var checks = getTodayChecks(), next = {}, k, ex = todayExercises()[i];
  for(k in checks){ next[k] = checks[k]; }
  next[i] = !checks[i];
  setTodayChecks(next);
...
function startWorkout(){
  var ex = todayExercises(), checks = getTodayChecks(), idx = 0, i;
  for(i=0;i<ex.length;i++){ if(!checks[i]){ idx = i; break; } }
...
function markAndNext(){
  var exs = todayExercises(), i = state.currentIndex, checks = getTodayChecks(), next = {}, k;
  for(k in checks){ next[k] = checks[k]; }
  next[i] = true;
  setTodayChecks(next);
...
function simpleLineChart(values, color){
  ...
  var range = max - min || 1;
  var points = [], idx, v, x, y;
  for(idx=0; idx<values.length; idx++){
    v = values[idx];
    if(typeof v !== "number") continue;
    x = (idx / Math.max(values.length - 1, 1)) * (width - 20) + 10;
    y = height - (((v - min) / range) * (height - 30) + 15);
    points.push(x + "," + y);
  }
...
function renderAchievements(){
  var items = achievements(), out = '', i;
  for(i=0;i<items.length;i++){
    out += '<div class="card"><div class="row-between"><div style="font-weight:900;">' + items[i].name + '</div><div class="badge ' + (items[i].unlocked ? 'badge-green' : 'badge-gray') + '">' + (items[i].unlocked ? 'Liberada' : 'Bloqueada') + '</div></div></div>';
  }
  return out;
}
...
function renderGuideOnly(){
  ...
  guide.innerHTML =
    '<div class="row-between"><div><div class="exercise-name">' + ex.name + '</div><div class="muted">' + ex.target + '</div></div><div class="badge ' + (done ? 'badge-green' : 'badge-gray') + '">' + (done ? 'Feito' : ((i + 1) + '/' + exs.length)) + '</div></div>' +
    renderVideoEmbed(ex) +
    '<div class="tip">' + ex.tip + '</div>' +
    '<div class="timer">' + formatTime(state.timerSeconds) + '</div><div class="small" style="text-align:center">Timer de descanso</div>' +
...
function renderInicio(){
  ...
  return '<div class="onboarding"><div class="small">PERSONALIZACAO</div><div class="grid-2" style="margin-top:10px;"><input id="profileName" type="text" value="' + state.profileName + '" placeholder="Seu nome" /><input id="profileGoal" type="text" value="' + state.profileGoal + '" placeholder="Sua meta" /></div><button class="btn btn-ghost" style="margin-top:10px;" onclick="saveProfile()">Salvar perfil</button></div>' +
    '<div class="glow-card"><div class="label">FOCO DO DIA</div><div class="plan-title">' + todayPlan() + '</div><div class="plan-sub">' + weekdayLabel() + ' • ' + state.profileGoal + '</div><div class="progress-bar"><div class="progress-fill" style="width:' + progressToday() + '%"></div></div><div class="row-between" style="margin-top:10px;"><div class="small" style="color:#ffd9b3">Progresso de hoje</div><div class="badge ' + (todayPlan() === "Descanso" ? 'badge-rest' : perfectToday ? 'badge-green' : 'badge-orange') + '">' + (todayPlan() === "Descanso" ? 'Descanso' : perfectToday ? 'Completo' : (progressToday() + '%')) + '</div></div><button class="btn btn-primary" style="margin-top:14px;" onclick="startWorkout()">Começar treino</button></div>' +
    '<div class="metric-grid" style="margin-top:12px;"><div class="metric"><div class="metric-label">Peso atual</div><div class="metric-value">' + safeVal(latest.weight,' kg') + '</div></div><div class="metric"><div class="metric-label">Cintura</div><div class="metric-value">' + safeVal(latest.waist,' cm') + '</div></div></div>' +
...
function renderHabitos(){
  ...
  var out = '<div class="card"><div style="font-size:18px;font-weight:900;">Check diário</div><div class="muted">Marque dieta, água e sono.</div>', i, key, label;
  for(i=0;i<items.length;i++){
    key = items[i];
    label = key === 'dieta' ? 'Segui a dieta' : key === 'agua' ? 'Bebi água suficiente' : 'Dormi bem';
...
function renderProgresso(){
  ...
  out += '<div class="chart"><div style="font-weight:900;">Gráfico de peso</div>' + simpleLineChart(weights, '#ff8a00') + '<div class="chart-labels">' + labels.map(function(l){ return '<div>' + l + '</div>'; }).join('') + '</div></div>';
  out += '<div class="chart"><div style="font-weight:900;">Gráfico de cintura</div>' + simpleLineChart(waists, '#22c55e') + '<div class="chart-labels">' + labels.map(function(l){ return '<div>' + l + '</div>'; }).join('') + '</div></div>';
...
function renderSemana(){
  var days = ["Seg","Ter","Qua","Qui","Sex","Sab","Dom"], out = '<div class="card"><div style="font-size:18px;font-weight:900;">Treinos da semana</div><div class="muted">Troque qualquer dia quando precisar.</div></div><div class="list">', i, day, current, j, opt;
  for(i=0;i<days.length;i++){
    day = days[i];
    current = state.weekPlan[day] || "Treino A";
    ...
    for(j=0;j<planOptions.length;j++){
      opt = planOptions[j];
...
function render(){
  document.getElementById("app").innerHTML =
    '<div class="hero"><div class="top-row"><div class="avatar">' + (state.profileName || 'L').charAt(0).toUpperCase() + '</div><div style="flex:1;"><div class="small" style="color:#ffcf9f">APP PESSOAL</div><h1 class="hero-title">' + state.profileName + ', foco em <span class="orange">resultado</span></h1><p class="hero-sub">' + state.profileGoal + '</p></div></div></div>' +
