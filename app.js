...
const KEY = "treino_v8_2_full_restore_pwa";
...
function nextExercise(){
  const ex = todayExercises();
  if(state.currentIndex < ex.length - 1){ state.currentIndex += 1; saveState(); renderGuideOnly(); }
}
function prevExercise(){
  if(state.currentIndex > 0){ state.currentIndex -= 1; saveState(); renderGuideOnly(); }
}
